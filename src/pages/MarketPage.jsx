...
function Tile({ value, label, sub }) {
  return (
    <div style={{ background: 'white', border: '1px solid #eeddd8', borderRadius: '14px', padding: '1.1rem 0.75rem', textAlign: 'center' }}>
      <div style={{ fontFamily: SERIF, fontSize: 'clamp(1.4rem, 4vw, 2rem)', fontWeight: 800, color: '#1c2b29', lineHeight: 1.1 }}>{value}</div>
      ...
      {sub && <div style={{ fontSize: '0.75rem', color: '#98a3a1', marginTop: '0.15rem' }}>{sub}</div>}
    </div>
  );
}
...
function TotalsBand({ year }) {
  const t = yearTotals();
  return (
    <div style={{ background: '#1a554e', color: 'white', borderRadius: '16px', padding: '1.1rem 1.5rem', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '1.25rem 2rem', marginBottom: '1.25rem' }}>
      <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#f5b5c7', textTransform: 'uppercase', letterSpacing: '1px', flexBasis: '100%' }}>Holly across the Irish Hills, {year}</div>
      ...
      <Link to="/sold" style={{ marginLeft: 'auto', color: '#f5b5c7', fontSize: '0.82rem', fontWeight: 700, textDecoration: 'none' }}>Every sale →</Link>
    </div>
  );
}
...
function MonthChart({ byMonth, lakeName, regionName, hasLakeData }) {
  ...
  return (
    <div style={{ background: 'white', border: '1px solid #eeddd8', borderRadius: '16px', padding: '1.25rem 1.25rem 0.9rem' }}>
      <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#98a3a1', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.9rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '5px', height: '90px' }}>
        {byMonth.map((m, i) => (
          <div key={m.month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%' }}>
            {m[key] > 0 && <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#1c2b29', marginBottom: '3px' }}>{m[key]}</div>}
            <div style={{ width: '100%', height: `${Math.max(4, (m[key] / max) * 64)}px`, background: i > now ? '#f6e9e5' : m[key] ? '#e64774' : '#eeddd8', borderRadius: '4px 4px 0 0' }} />
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: '5px', marginTop: '0.4rem' }}>
        {byMonth.map((m) => <div key={m.month} style={{ flex: 1, textAlign: 'center', fontSize: '0.75rem', color: '#98a3a1', fontWeight: 600 }}>{m.month[0]}</div>)}
      </div>
    </div>
  );
}
...
function SaleRow({ p }) {
  ...
  return (
    <Link to={`/property/${p.slug}`} style={{ display: 'flex', gap: '0.85rem', alignItems: 'center', padding: '0.7rem 0', borderTop: '1px solid #f6e9e5', textDecoration: 'none', color: '#1c2b29' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: '0.75rem', color: '#66706e' }}>{soldBadge(p)} · {new Date(`${p.soldOn}T12:00:00Z`).toLocaleDateString('en-US', { month: 'short', year: 'numeric', timeZone: 'UTC' })}</div>
      </div>
...
export default function MarketPage() {
    ...
    return (
      <Shell title="Market reports | Holly Griewahn">
        <div style={{ padding: '8rem 2rem', textAlign: 'center' }}>
          ...
          <Link to="/market" style={{ color: '#e64774', fontWeight: 700 }}>See all lake reports</Link>
        </div>
      </Shell>
    );
  }
  ...
  return (
    <Shell title={`${lake.name} sales report ${year} | Holly Griewahn`}>
      <div style={{ minHeight: '46vh', background: 'linear-gradient(135deg, #1a554e 0%, #237168 60%, #174a44 100%)', display: 'flex', alignItems: 'flex-end', padding: '7rem 2rem 2.5rem', position: 'relative' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '960px', margin: '0 auto', width: '100%' }}>
          ...
          <p style={{ color: '#f5b5c7', fontSize: '0.78rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '0.6rem' }}>{year} sales report</p>
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto', padding: '2rem 1.5rem 5rem' }}>
        ...
        <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#98a3a1', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.6rem' }}>{hasLakeData ? `Of those, on ${lake.name}` : `${region.name}, the market ${lake.name} is priced against`}</div>
        ...
        <div className="mk-two" style={{ marginBottom: '1.25rem' }}>
          ...
          <div style={{ background: 'white', border: '1px solid #eeddd8', borderRadius: '16px', padding: '1.25rem 1.5rem' }}>
            <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#98a3a1', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.6rem' }}>What this means if you own on {lake.name}</div>
            <p style={{ fontSize: '0.9rem', lineHeight: 1.65, color: '#4a5654' }}>
              {s.dayOne > 0
            ...
            <Link to="/cma" style={{ display: 'inline-block', marginTop: '0.9rem', background: '#e64774', color: 'white', padding: '0.7rem 1.2rem', borderRadius: '10px', textDecoration: 'none', fontWeight: 700, fontSize: '0.88rem' }}>What's my {lake.name} home worth?</Link>
          </div>
        </div>

        {(hasLakeData ? lakeSold : regionSold).length > 0 && (
          <div style={{ background: 'white', border: '1px solid #eeddd8', borderRadius: '16px', padding: '1.25rem 1.5rem', marginBottom: '1.25rem' }}>
        ...
        {forSale.length > 0 && (
          <div style={{ background: 'white', border: '1px solid #eeddd8', borderRadius: '16px', padding: '1.25rem 1.5rem', marginBottom: '1.25rem' }}>
            ...
            {forSale.map((p) => (
              <Link key={p.id} to={`/property/${p.slug}`} style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', padding: '0.6rem 0', borderTop: '1px solid #f6e9e5', textDecoration: 'none', color: '#1c2b29' }}>
                <span style={{ fontWeight: 600 }}>{p.title}</span><span style={{ fontWeight: 800, color: '#e64774' }}>{p.price}</span>
              </Link>
            ))}
          </div>
        )}
        ...
        <p style={{ fontSize: '0.75rem', color: '#98a3a1', marginTop: '1.25rem', lineHeight: 1.5 }}>
...
function MarketIndex() {
  ...
  const th = (key, label, align = 'right') => (
    <th onClick={() => setSort(key)} style={{ textAlign: align, padding: '0.6rem 0.75rem', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.6px', color: sort === key ? '#e64774' : '#98a3a1', cursor: 'pointer', whiteSpace: 'nowrap', borderBottom: '1px solid #eeddd8' }}>{label}{sort === key ? ' ▾' : ''}</th>
  );
  return (
    <Shell title={`Irish Hills lake sales reports ${year} | Holly Griewahn`}>
      <div style={{ background: 'linear-gradient(135deg, #1a554e 0%, #237168 60%, #174a44 100%)', padding: '7rem 2rem 2.5rem' }}>
        <div style={{ maxWidth: '960px', margin: '0 auto' }}>
          <p style={{ color: '#f5b5c7', fontSize: '0.78rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '0.6rem' }}>Lake by lake</p>
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto', padding: '2rem 1.5rem 5rem' }}>
        ...
        <div style={{ background: 'white', border: '1px solid #eeddd8', borderRadius: '16px', overflowX: 'auto' }}>
            ...
            <tbody>
              {rows.filter((r) => r.sold || r.active).map((r) => (
                <tr key={r.lake.slug} onClick={() => { window.location.href = `/market/${r.lake.slug}`; }} style={{ cursor: 'pointer' }}
                  onMouseEnter={(e) => { e.currentTarget.style.background = '#fdf7f5'; }} onMouseLeave={(e) => { e.currentTarget.style.background = 'white'; }}>
                  <td style={{ padding: '0.75rem', borderBottom: '1px solid #f6e9e5' }}><Link to={`/market/${r.lake.slug}`} style={{ color: '#1c2b29', fontWeight: 700, textDecoration: 'none' }}>{r.lake.name}</Link><div style={{ fontSize: '0.75rem', color: '#98a3a1' }}>{r.region?.name}</div></td>
                  <td style={{ padding: '0.75rem', textAlign: 'right', borderBottom: '1px solid #f6e9e5', fontFamily: SERIF, fontWeight: 800, fontSize: '1.15rem', color: r.sold ? '#e64774' : '#d5dcda' }}>{r.sold}</td>
                  <td style={{ padding: '0.75rem', textAlign: 'right', borderBottom: '1px solid #f6e9e5', fontWeight: 600 }}>{r.median ? fmtPrice(r.median) : <span style={{ color: '#d5dcda' }}>—</span>}</td>
                  <td style={{ padding: '0.75rem', textAlign: 'right', borderBottom: '1px solid #f6e9e5', fontWeight: 600 }}>{r.avgDays === null ? <span style={{ color: '#d5dcda' }}>—</span> : r.avgDays}</td>
                  <td style={{ padding: '0.75rem', textAlign: 'right', borderBottom: '1px solid #f6e9e5', fontWeight: 600 }}>{r.active || <span style={{ color: '#d5dcda' }}>—</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {rows.some((r) => !r.sold && !r.active) && (
          <div style={{ marginTop: '1.25rem' }}>
            <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#98a3a1', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.6rem' }}>Other lakes Holly covers</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {rows.filter((r) => !r.sold && !r.active).sort((a, b) => a.lake.name.localeCompare(b.lake.name)).map((r) => (
                <Link key={r.lake.slug} to={`/market/${r.lake.slug}`} style={{ background: 'white', border: '1px solid #eeddd8', borderRadius: '20px', padding: '0.4rem 0.9rem', fontSize: '0.82rem', fontWeight: 600, color: '#1c2b29', textDecoration: 'none' }}>{r.lake.name}</Link>
              ))}
            </div>
          </div>
        )}
        {t.unassigned > 0 && <p style={{ fontSize: '0.75rem', color: '#98a3a1', marginTop: '0.9rem' }}>{t.unassigned} of the {t.sold} sales are village, land or rural homes without a lake, or are waiting on lake confirmation, so lake counts add up to less than the total.</p>}
      </div>
    </Shell>
  );
}
