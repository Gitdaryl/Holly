...
const input = { width: '100%', padding: '0.65rem 0.8rem', borderRadius: '8px', border: '1px solid #eeddd8', fontSize: '0.9rem', fontFamily: 'inherit', outline: 'none', background: 'white' };
const label = { fontSize: '0.75rem', fontWeight: 700, color: '#66706e', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: '0.35rem' };
...
export default function LakeWaitlist({ lake, lakeName, compact = false, role = 'buyer' }) {
  ...
  return (
    <div id="waitlist" style={{ background: 'linear-gradient(135deg, #1a554e, #1f655c)', borderRadius: '16px', padding: compact ? '1.5rem' : '2rem', color: 'white', scrollMarginTop: '90px' }}>
      <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#f5b5c7', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.5rem' }}>{owner ? 'For owners' : 'Before it hits the MLS'}</div>
      <h2 style={{ fontFamily: "'Source Serif 4', Georgia, serif", fontSize: compact ? '1.3rem' : '1.5rem', fontWeight: 700, marginBottom: '0.5rem', lineHeight: 1.25 }}>{headline}</h2>
      ...
      {status === 'sent' ? (
        <div style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.18)', borderRadius: '12px', padding: '1.25rem' }}>
          ...
          <div style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.72)', lineHeight: 1.6 }}>
            Holly will reach out to learn exactly what you want. In a hurry? Call or text <a href="tel:5174033413" style={{ color: '#f5b5c7', fontWeight: 600, textDecoration: 'none' }}>(517) 403-3413</a>.
          </div>
        </div>
      ) : (
        <form onSubmit={submit} style={{ display: 'grid', gridTemplateColumns: compact ? '1fr' : 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
          ...
          <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <button type="submit" disabled={status === 'sending'} style={{ background: '#e64774', color: 'white', border: 'none', padding: '0.85rem 1.5rem', borderRadius: '10px', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', fontFamily: 'inherit', opacity: status === 'sending' ? 0.7 : 1 }}>
